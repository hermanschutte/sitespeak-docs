...
- **Cap response length**: send `response_token_limit` to bound how long the client's AI agents' answers can be. The agents are instructed to write complete answers that fit the budget, with a hard cut as the backstop. Length shaping is approximate and varies a little by model; treat the limit as a strong target rather than an exact word count.
...
| response_token_limit | Integer | Maximum response length in tokens (1 to 10000). Applies to new answers within seconds. Send `null` to remove the limit. |
