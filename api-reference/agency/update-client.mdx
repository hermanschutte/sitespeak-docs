...
- **Withhold integrations**: send `enabled_integrations` with the keys the client may use. Anything left out is disconnected for that client right away (their saved connection is removed) and hidden from their dashboard. Send `null` to allow everything again. See [Per-Client Integrations](/agency/client-management#per-client-integrations).
...
| enabled_integrations | Array | Which integrations this client may use: any of `whatsapp`, `hubspot`, `slack`, `google_chat`, `telegram`, `discord`, `actions`. Anything left out is disconnected for the client at once and hidden from their dashboard. Send `null` to allow everything your agency plan includes. See [Per-Client Integrations](/agency/client-management#per-client-integrations). |
