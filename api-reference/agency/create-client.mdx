...
| response_token_limit | Integer | Response length limit in tokens (50 to 10000) for this client's AI agents. Answers are shaped to fit, with a small allowance to finish the last sentence; `null` or omitted means no limit. |
| byok_enabled | Boolean | When `true`, the client's workspace administrators can add their own LLM API keys. Defaults to `false`. Messages still count against the client's credits. See [Client API Keys (BYOK)](/agency/client-management#client-api-keys-byok). |
| enabled_integrations | Array | Which integrations this client may use: any of `whatsapp`, `hubspot`, `slack`, `google_chat`, `telegram`, `discord`, `actions`. Omit or `null` to allow everything your agency plan includes. An empty array withholds all of them. See [Per-Client Integrations](/agency/client-management#per-client-integrations). |
...
curl -X POST "https://api.sitespeak.ai/v1/agency/clients" \
  ...
  -d '{
    ...
    "enabled_integrations": ["slack", "hubspot", "actions"],
    "chatbot": {
