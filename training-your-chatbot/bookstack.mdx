...
- Make sure the role of the user who created the API token has the **Access system API** permission enabled (BookStack **Settings → Roles**). Without it, BookStack rejects every request with "The owner of the used API token does not have permission to make API calls"
- Check your BookStack role permissions
- Verify the pages aren't restricted to specific roles
- Try creating a new API token with broader permissions, or create the token as an admin user
