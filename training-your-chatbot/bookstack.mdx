...
Once connected, SiteSpeakAI imports every BookStack page your API token's user can view. Each page appears as its own source:

1. **Automatic import**: All pages visible to the token's user are fetched and processed
2. **Monitor status**: Watch the training progress in the Sources list
3. **Control what's included**: Use BookStack's role and content permissions to limit which pages the token's user (and so your chatbot) can see
...
- Make sure the role of the user who created the API token has both the **Access system API** and **Export content** permissions enabled (BookStack **Settings → Roles**). Without them, BookStack rejects the API or page-export requests and no content is imported
- Check your BookStack role permissions
- Verify the pages aren't restricted to specific roles
- Try creating a new API token whose role has the **Access system API** and **Export content** permissions, plus view access to the books you want imported
