...
The **Send Transcript** action enables your AI agent to send a copy of the conversation to the visitor via email. When invoked, it collects the visitor's email address (or uses a previously known email) and delivers the full conversation thread.
...
When a visitor asks for a copy of the conversation, your chatbot invokes the Send Transcript action:

- **If the visitor's email is already known** (e.g., from a previous lead capture or identification), the transcript is sent automatically with a brief confirmation.
- **If the visitor's email is not known**, a simple email input form is displayed. After submission, the transcript is sent and their email is saved to Contacts.
