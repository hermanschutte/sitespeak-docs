...
No additional configuration is required. As soon as a visitor is identified through `window.SiteSpeakAI.identify({ token })`, every conversation they have is linked to their `user_id` and appears in their Recent Chats list on any device they sign in from.
